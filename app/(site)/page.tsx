...
export const revalidate = 60; // ISR cache validation
...
export default function HomePage() {
  ...
  const faqItems = (() => {
    try {
      const parsed = JSON.parse(content.faq_json) as Array<{
        question: string;
        answer: string;
      }>;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-12 px-4 pb-8">
      <section className="glass glow fade-up rounded-3xl p-8 md:p-12">
        ...
        <h1
          className="text-3xl font-bold leading-tight md:text-5xl text-gradient"
      ...
      <section className="space-y-4" id="services">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {services.map((service) => (
            <article
              key={service.id}
              className="glass glow hover-lift rounded-2xl overflow-hidden"
            >
              <Image
              ...
              <div className="p-4">
                ...
                <Link
                  ...
                  className="mt-4 inline-block text-sm text-cyan-200 hover:text-cyan-100"
                >
                  Подробнее →
                </Link>
              </div>
      ...
      <section className="space-y-4" id="pricing">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {pricing.map((item, index) => (
            <article
              key={item.id}
              className="glass price-card hover-lift rounded-2xl p-5"
            >
      ...
      <section className="grid gap-4 lg:grid-cols-2" id="about">
        ...
        <article className="glass rounded-2xl p-6" id="contacts">
          ...
          <div className="mt-3 space-y-2 text-slate-200">
            ...
            <p>
              Email:{" "}
              <a className="text-cyan-200" href={`mailto:${settings.email}`}>
                {settings.email}
              </a>
            </p>
