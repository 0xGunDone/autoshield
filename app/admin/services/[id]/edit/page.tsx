...
export default async function EditServicePage({ params }: Props) {
  ...
  return (
    <AdminShell title={`Редактирование: ${service.title}`}>
      <form action={`/api/admin/services/${service.id}`} method="post" className="glass rounded-2xl p-5 space-y-4">
        ...
        <div>
          ...
          <textarea name="short_description" rows={3} defaultValue={service.short_description} required />
        </div>

        <div>
          <label className="mb-1 block text-sm">Подробное описание</label>
          <textarea name="description" rows={6} defaultValue={service.description} required />
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="mb-1 block text-sm">Цена от</label>
            <input name="price_from" defaultValue={service.price_from} required />
          </div>
          <div>
            <label className="mb-1 block text-sm">Изображение (URL)</label>
            <input name="image_url" defaultValue={service.image_url} required />
          </div>
        </div>
        ...
        <div>
          ...
          <input name="seo_description" defaultValue={service.seo_description} required />
        </div>
      ...
      <form action={`/api/admin/services/${service.id}`} method="post" className="glass rounded-xl p-4">
